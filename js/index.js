...
(() => {
  ...
  // ts/photoloader.ts
  var API = "https://webetu.iutnc.univ-lorraine.fr";
  ...
  function loadPicture(id2) {
    return loadResource(`/www/canals5/phox/api/photos/${id2}`);
  }
  function loadGallery() {
    return loadResource("/www/canals5/phox/api/photos");
  }

  // ts/ui.ts
  var import_handlebars = __toESM(require_handlebars());
  var _a, _b;
  var pictureSource = (_b = (_a = document.querySelector("#pictureTemplate")) == null ? void 0 : _a.innerHTML) != null ? _b : "";
  var pictureTemplate = import_handlebars.default.compile(pictureSource);
  ...
  function displayCategorie(data) {
    ...
    if (zone) {
      zone.innerHTML = `<h3>Cat\xE9gorie</h3><p>${data.categorie.nom}</p>`;
    }
  }
  ...
  var _a2, _b2;
  var gallerySource = (_b2 = (_a2 = document.querySelector("#galleryTemplate")) == null ? void 0 : _a2.innerHTML) != null ? _b2 : "";
  var galleryTemplate = import_handlebars2.default.compile(gallerySource);
  ...
  function next() {
    ...
    return loadResource(actuel.links.next.href).then(storeGallery);
  }
  function prev() {
    if (!actuel.links.prev) {
      return Promise.reject(new Error("Pas de page pr\xE9c\xE9dente"));
    }
    return loadResource(actuel.links.prev.href).then(storeGallery);
  }
  function first() {
    if (!actuel.links.first) {
      return Promise.reject(new Error("Pas de premi\xE8re page"));
    }
    return loadResource(actuel.links.first.href).then(storeGallery);
  }
  function last() {
    if (!actuel.links.last) {
      return Promise.reject(new Error("Pas de derni\xE8re page"));
    }
    return loadResource(actuel.links.last.href).then(storeGallery);
  }

  // ts/index.ts
  function loadCategory(photo) {
  ...
  var id = hash ? parseInt(hash) : 105;
  getPicture(id);
  function showGallery() {
    load().then(displayGallery).catch((error) => {
      console.error(error);
    });
  }
  var loadButton = document.querySelector("#charger_gallery");
  if (loadButton) {
  ...
  if (nextButton) {
    nextButton.addEventListener("click", () => {
      next().then(displayGallery).catch((error) => {
        console.error(error);
      });
    });
  }
  ...
  if (prevButton) {
    prevButton.addEventListener("click", () => {
      prev().then(displayGallery).catch((error) => {
        console.error(error);
      });
    });
  }
  ...
  if (firstButton) {
    firstButton.addEventListener("click", () => {
      first().then(displayGallery).catch((error) => {
        console.error(error);
      });
    });
  }
  ...
  if (lastButton) {
    lastButton.addEventListener("click", () => {
      last().then(displayGallery).catch((error) => {
        console.error(error);
      });
    });
  }
